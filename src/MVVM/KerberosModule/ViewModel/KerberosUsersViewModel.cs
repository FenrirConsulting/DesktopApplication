...
{
    public class KerberosUsersViewModel : BaseViewModel
    {
        #region Delegates
        ...
        private KerberosUsersUpdateRecordViewModel usersUpdateRecordVm;
        public KerberosUsersUpdateRecordViewModel UsersUpdateRecordVm
        {
            get { return usersUpdateRecordVm; }
            set { usersUpdateRecordVm = value; OnPropertyChanged(); }
        }

        // Users Add Record View
        private KerberosUsersAddRecordViewModel usersAddRecordVm;
        public KerberosUsersAddRecordViewModel UsersAddRecordVm
        {
        ...
        private readonly ObservableCollection<SearchTerms> searchTerms = new()
        {
            ...
            new SearchTerms { SearchTerm = "Employee ID" },
        ...
        public async void OnLoad()
        {
            ...
            {
                NoFoundResults = false;
                if (MainViewModel.KerberosUsersTable.Rows.Count < 10)
                {
                    MainViewModel.KerberosUsersTable.Clear();
                    IsLoadBool = true;
                    var dataTask = await DBConn.GetTableAsync(ConfigurationProperties.KerberosUsersTable);
                    MainViewModel.KerberosUsersTable = dataTask;
                }
                BindTableBuild();
                IsLoadBool = false;
            }
        ...
        {
            BindTable.Clear();
            BindTable = MainViewModel.KerberosUsersTable.Copy();
        ...
        public async void LoadDatabase()
        {
            ...
            {
                MainViewModel.KerberosUsersTable.Clear();
                DisplayView = MainViewModel.KerberosUsersTable.DefaultView;
                IsLoadBool = true;
                var dataTask = await DBConn.GetTableAsync(ConfigurationProperties.KerberosUsersTable);
                MainViewModel.KerberosUsersTable = dataTask;
                IsLoadBool = false;
        ...
        private void SearchFilter()
        {
            if (SearchText != "Search" && DisplayTable.Rows.Count > 0)
            {
                try
                {
                    ...
                    {
                        case "All":
                            builtFilter = "iamhku_uid + iamhku_logid + iamhku_empid + iamhku_adid + iamhku_name + iamhku_comments + iamhku_servers Like '%" + SearchText + "%'";
                            break;
                        ...
                        case "UID":
                            ...
                            selectedColumn = "iamhku_uid";
                            builtFilter = selectedColumn + " = '" + SearchText + "'";
                            break;

                        case "Logon ID":
                            selectedColumn = "iamhku_logid";
                            builtFilter = selectedColumn + " Like '%" + SearchText + "%'";
                            break;

                        case "Employee ID":
                            selectedColumn = "iamhku_empid";
                            builtFilter = selectedColumn + " Like '%" + SearchText + "%'";
                            break;

                        case "AD ID":
                            selectedColumn = "iamhku_adid";
                            builtFilter = selectedColumn + " Like '%" + SearchText + "%'";
                            break;

                        case "Name":
                            selectedColumn = "iamhku_name";
                            builtFilter = selectedColumn + " Like '%" + SearchText + "%'";
                            break;

                        case "Comments":
                            selectedColumn = "iamhku_comments";
                            builtFilter = selectedColumn + " Like '%" + SearchText + "%'";
                            break;

                        case "Servers":
                            selectedColumn = "iamhku_servers";
                            builtFilter = selectedColumn + " Like '%" + SearchText + "%'";
                            break;
                    }

                    string sort = "iamhku_uid";
            ...
            try
            {
                if (MessageBox.Show("Are you certain you want to delete this record?", "Delete Record??", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
                {
                    ...
                    if (drv != null)
                    {
                        ...
                        if (row != null)
                        {
                            string record = row["iamhku_id"].ToString();
                            string UpdateValue = record;
                            DBConn.DeleteMatchingRecords(ConfigurationProperties.KerberosUsersTable, "iamhku_id", UpdateValue);
                            List<DataRow> rowsToDelete = new();
                            foreach (DataRow dr in MainViewModel.KerberosUsersTable.Rows)
                            {
                                if (dr["iamhku_id"].ToString() == UpdateValue)
                                {
                                    rowsToDelete.Add(dr);
                                }
                            }
        ...
        public List<String> BuildExportList()
        {
            ...
            if (ExportColumnList.Column1IsChecked == true) { exportList.Add("iamhku_id"); };
            if (ExportColumnList.Column2IsChecked == true) { exportList.Add("iamhku_uid"); };
            if (ExportColumnList.Column3IsChecked == true) { exportList.Add("iamhku_logid"); };
            if (ExportColumnList.Column4IsChecked == true) { exportList.Add("iamhku_empid"); };
            if (ExportColumnList.Column5IsChecked == true) { exportList.Add("iamhku_adid"); };
            if (ExportColumnList.Column6IsChecked == true) { exportList.Add("iamhku_name"); };
            if (ExportColumnList.Column7IsChecked == true) { exportList.Add("iamhku_comments"); };
            if (ExportColumnList.Column8IsChecked == true) { exportList.Add("iamhku_servers"); };
            if (ExportColumnList.Column9IsChecked == true) { exportList.Add("iamhku_history"); };
            return exportList;
        }
        ...
        public List<String> BuildRemoveExportList()
        {
            ...
            if (ExportColumnList.Column1IsChecked == false) { exportRemoveList.Add("iamhku_id"); };
            if (ExportColumnList.Column2IsChecked == false) { exportRemoveList.Add("iamhku_uid"); };
            if (ExportColumnList.Column3IsChecked == false) { exportRemoveList.Add("iamhku_logid"); };
            if (ExportColumnList.Column4IsChecked == false) { exportRemoveList.Add("iamhku_empid"); };
            if (ExportColumnList.Column5IsChecked == false) { exportRemoveList.Add("iamhku_adid"); };
            if (ExportColumnList.Column6IsChecked == false) { exportRemoveList.Add("iamhku_name"); };
            if (ExportColumnList.Column7IsChecked == false) { exportRemoveList.Add("iamhku_comments"); };
            if (ExportColumnList.Column8IsChecked == false) { exportRemoveList.Add("iamhku_servers"); };
            if (ExportColumnList.Column9IsChecked == false) { exportRemoveList.Add("iamhku_history"); };
        ...
        private void HtmlExport()
        {
            try
            {
                ...
                dlg.FileName = "KerberosUsersExport";
                dlg.DefaultExt = ".html";
        ...
        private void ExcelExportMethod()
        {
            try
            {
                ...
                else
                {
                    ...
                    XLWorkbook wb = new();
                    string exportName = "KerberosUsersDatabase";
                    wb.Worksheets.Add(ExportTable, exportName);
                    var saveFileDialog = new SaveFileDialog
                    {
                        Filter = "Excel files|*.xlsx",
                        Title = "Save an Excel File",
                        FileName = "KerberosUsersExcelBackup.xlsx"
                    };
        ...
        private void RecordSelectMethod()
        {
            try
            {
                ...
                if (drv != null)
                {
                    ...
                    if (row != null)
                    {
                        UsersUpdateRecordVm = null;
                        SelectedViewModel = null;
                        string record = row["iamhku_id"].ToString();
                        KerberosUsersViewModel.ChosenRecord = record;
                        UsersUpdateRecordVm = new KerberosUsersUpdateRecordViewModel();
                        UsersUpdateRecordVm.LoadData();
                        SelectedViewModel = UsersUpdateRecordVm;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void AddRecord()
        {
            try
            {
                ...
                SelectedViewModel = null;
                UsersAddRecordVm = new KerberosUsersAddRecordViewModel();
        ...
        {
            if (passedMessage == "Kerberos Users Table Reload") { BindTableBuild(); ; }
        }
        #endregion
    }
}
